...
class StackingAveragedModels(BaseEstimator):
    ...
    def fit(self, X, y):
        X_ = X.reset_index()
        y_ = y.reset_index()
        X_.drop("id", axis=1, inplace=True)
        y_.drop("id", axis=1, inplace=True)
        self.base_models_ = [list() for k in self.base_models]
        ...
        for i, model in enumerate(self.base_models):
            for train_index, holdout_index in kfold.split(X_, y_):
                ...
                instance.fit(X_.iloc[train_index, :], y_.iloc[train_index,:])
                y_pred = instance.predict(X_.iloc[holdout_index, :])
